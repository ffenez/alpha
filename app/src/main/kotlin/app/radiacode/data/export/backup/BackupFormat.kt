package app.radiacode.data.export.backup
 ...
 *
 * Двоичная копия Room-базы намертво привязана к её схеме: восстановить её
 * можно только приложением с ровно той же версией схемы, а мигрировать чужую
 * базу вслепую — способ потерять данные молча. Поэтому у копии СВОЯ версия
 * формата, независимая от версии базы: она описывает то, что человек считает
 * своими данными (измерения, профили, маршруты, спектры, эксперименты,
 * настройки), а не то, как они сейчас разложены по таблицам.
 *
 * ## Что внутри
 *
 * Обычный ZIP с явным манифестом; расширение только называет его копией
 * приложения. Крупные ряды — построчный JSON (NDJSON): запись пишется и
 * читается по одной строке, поэтому ни экспорт, ни восстановление не держат
 * историю в памяти целиком.
 *
 * ```text
 ...
 * ПЕРЕСЧИТЫВАЮТСЯ из измерений, и класть их в копию значит удваивать её размер
 * ради того, что приложение построит само. Кэша карт, журналов сбоев и
 * системных разрешений — тоже: первое не данные, второе диагностика, третье
 * невозможно перенести честно.
 */
object BackupFormat {

    /** Что это за файл — первое, что читает импорт. */
    const val FORMAT = "radiacode-backup"
    ...
    /** Расширение файла, которое видит человек. */
    ...
    fun fileName(dateIso: String): String = "RadiaCode-backup-$dateIso.$EXTENSION"
}
...
data class BackupManifest(
    ...
    /** Момент создания, ISO-8601 с зоной — как его прочтёт человек. */
...
/** Что человек согласился положить в копию (и что в ней действительно есть). */
...
/**
 * Контрольные суммы записей архива.
 *
 * Проверяются ДО восстановления и все сразу: частичное молчаливое
 * восстановление испорченной копии хуже отказа — человек считает, что данные
 * вернулись, а вернулась их половина.
 */
