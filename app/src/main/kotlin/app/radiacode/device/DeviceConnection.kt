...
class DeviceConnection private constructor(
     ...
     * уезжали в БУДУЩЕЕ на полторы минуты.
     *
     * Прибор пишет раз в секунду и отдаёт написанное по первому запросу,
     * поэтому САМАЯ СВЕЖАЯ запись ответа сделана практически в момент, когда
     * мы этот ответ получили. Отсюда поправка считается прямо, одним
     * вычитанием, и заново на каждом ответе с новой записью: база не
     * «сходится» и не «дрейфует», она просто равна наблюдению. Более ранние
     * записи того же ответа сохраняют свои интервалы — сдвигается ВЕСЬ ответ
     * целиком, а не каждая запись по отдельности.
     *
     * ## Почему не фильтр по окну
     *
     * Здесь был min-фильтр с окном и предохранителями — классика синхронизации
     * часов. Он верен, но у него есть время сходимости, и ровно это время
     * экран говорил «нет новых данных», а метки записей лежали в прошлом,
     * из-за чего свежие строки натыкались на уникальный индекс
     * `samples.timestamp` и пропадали. Прямой якорь сходится за один ответ.
     *
     * ## Единственный предохранитель
     *
     * Пере-якорение делается ТОЛЬКО когда новейшая запись ответа отличается от
     * прежней. Если прибор перестал писать, он повторяет последнюю запись из
     * ответа в ответ; двигать по ней базу означало бы штамповать старое
     * показание текущим временем. Признак — «смещение изменилось», а не
     * «выросло»: буферная запись может лежать впереди живых, и требование
     * роста заперло бы базу в её эпохе.
     */
    @Volatile
