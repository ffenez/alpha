...
data class ChartGesture(
    ...
    /**
     * Пора ли пересобирать кадр прямо посреди жеста.
     *
     * Отдаление уводит видимое окно за нарисованный диапазон почти сразу: уже
     * вдвое разведённые пальцы требуют геометрии, которой нет. Пересобирать её
     * на КАЖДОЕ событие указателя — это шестьдесят пересборок в секунду, то
     * есть ровно та работа, ради избавления от которой существуют два окна.
     *
     * Поэтому пересборка на ходу ограничена по частоте: не чаще одной за
     * [MIN_COMMIT_INTERVAL_MILLIS]. Между ними по краям кадра видна пустота —
     * честная: там ещё не посчитано. Глаз при быстром отдалении её не читает
     * как дефект, а рывки читает всегда.
     */
