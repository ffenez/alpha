...
/**
 * Прибор, с которым приложение когда-либо работало.
 *
 * ## Зачем таблица, а не одна строка настроек
 *
 * Приборов у человека бывает несколько: домашний, дачный, походный. Каждый из
 * них продолжает записывать автономно, пока подключён другой, и у каждого своя
 * история, свой кристалл и своё разрешение. Пока «прибор» был одной строкой
 * адреса в настройках, второй прибор было не назвать, не отличить и не выбрать.
 *
 * ## Ключ — серийник
 *
 * Серийник приходит от самого прибора, в отличие от BLE-адреса (у части
 * телефонов он случайный) и имени в эфире (его можно переименовать). Им же
 * помечены измерения, снимки спектра и шаблоны, поэтому отдельного
 * искусственного ключа не заводится: он потребовал бы переписать уже
 * записанное.
 *
 * ## Имя
 *
 * [displayName] задаёт человек. Серийник — техническая подробность: он
 * показывается там, где спрашивают именно его, а не вместо имени.
 */
@Entity(
    tableName = "devices",
...
data class DeviceEntity(
    ...
    /** Имя, данное человеком; null — показывается модель. */
    val displayName: String? = null,
    /** Модель по серийнику на момент последней встречи. */
    val model: String? = null,
    val firmware: String? = null,
    /** Последний известный BLE-адрес: по нему приложение к нему возвращается. */
