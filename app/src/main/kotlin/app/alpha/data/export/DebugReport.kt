 ...
 *
 * Расход энергии из частоты не выводится: он зависит от размера ответа,
 * интервала BLE-соединения и числа пробуждений. В отчёте лежат факты
 * (запросов, байт, время работы), вывода про батарею нет.
 */
data class SpectrumTraffic(
    /** Выбранная ступень частоты, [app.alpha.data.SpectrumPollPolicy.id]. */
    val policy: String,
    val requests: Long,
...
data class DebugSnapshot(
    ...
    /** Жив ли цикл перечитывания графиков Главной. */
...
/**
 * Текстовый отчёт о состоянии приложения для разбора полевых наблюдений.
 *
 * Отчёт описывает состояние, а не измерения: в нём нет ни координат, ни рядов
 * отсчётов, ни содержимого спектров. Это сказано в самом файле.
 *
 * Строки статуса берутся те же, что показаны на экране.
 */
object DebugReport {
    ...
    fun fileName(nowMillis: Long, stamp: (Long) -> String): String =
        "alpha-debug-${stamp(nowMillis)}.txt"
