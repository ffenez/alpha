...
class ProfileRepository(
     ...
     *
     * «В пути» и «Без места» — места хранения для двух решений машины
     * контекста: без них измерения этих состояний остаются без профиля.
     * Восстанавливаются по РОЛИ, а не по имени — профиль можно переименовать.
     *
     * «Дом» создаётся только на пустой таблице: это стартовое предположение, и
     * его можно удалить.
     */
     ...
     * token, never an SSID (see [app.alpha.context.NetworkIdentity]);
