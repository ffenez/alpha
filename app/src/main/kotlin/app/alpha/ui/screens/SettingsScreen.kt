        ...
        entries += DebugBundle.Entry(
            ...
            content = RcXml.write(
                RcResultData(
                    ...
                    spectrum = SpectrumExport.toRcSpectrum(entity, "background"),
                    background = null,
                ),
            ),
        )
...
private fun HomeLayoutSection(graph: AppGraph) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(Dimens.space3)) {
        ...
        SettingsSection(title = strings.monitorBlocksNote) {
            ...
            // второстепенных.
            SwitchSettingRow(
                title = strings.blockDoseChart,
                checked = blocks.doseChart,
                onChange = { on ->
                    scope.launch { graph.settings.setMonitorBlocks(blocks.copy(doseChart = on)) }
                },
            )
            SettingsDivider()
            SwitchSettingRow(
