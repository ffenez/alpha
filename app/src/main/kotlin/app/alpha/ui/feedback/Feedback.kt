...
object Feedback {
    ...
    /**
     * Пауза-толчок-пауза-толчок, мс.
     *
     * **Инженерный параметр**: два коротких толчка по 90 мс с промежутком
     * 110 мс. Двойной толчок узнаётся как сигнал (одиночный можно принять за
     * что угодно), но остаётся коротким: тревога сообщает о событии, а не
     * трясёт телефон. С поисковым пульсом (одиночные 40 мс) не сливается ни по
     * длине, ни по числу.
     */
    private val ALARM_PATTERN_MILLIS =
        longArrayOf(0L, 90L, 110L, 90L)
