...
object RuStrings : Strings {
    ...
    override val readingHelpTitle = "Что значат эти числа"
    override val readingHelpDoseRate =
        "Мощность дозы, мкЗв/ч — темп, с которым набирается доза: сколько набежало бы за час, " +
            ...
            "мест — 0,08–0,20 мкЗв/ч, то есть 2–5 мкЗв за сутки."
    override val readingHelpCountRate =
        "Скорость счёта, имп/с — сколько частиц прибор зарегистрировал за секунду. Она отзывчивее: " +
            "меняется за секунды, а мощность дозы усредняется дольше, поэтому в поиске ведут по " +
            "ней. Зато она зависит от кристалла прибора и от энергии излучения — с чужим прибором " +
            "её не сравнить."
    override val readingHelpNoise =
        "Оба числа скачут сами по себе: распад случаен, и соседние секунды различаются даже при " +
            "неизменном уровне. Поэтому смысл имеет не одно значение, а то, держится ли оно: " +
            "приложение сравнивает окна измерений и говорит о разнице только тогда, когда её " +
            "подтверждает критерий."
    override val readingHelpScale =
        "Шкала под числом показывает отношение к тому, с чем идёт сравнение: в наблюдении — к " +
            "обычному для этого места, в поиске — к поставленной точке отсчёта, а пока её нет — к " +
            "недавнему уровню. Знаменатель всегда назван рядом со шкалой."
    ...
    // Вывод читается мельком: два-три слова, из которых понятно, с чем
    // сравнили. «Порог тревоги» назван ВАШИМ — он задан в настройках.
    ...
    // Вторая строка называет, с чем сравнили. «P10–P90», «L1» и «наблюдений»
    // — внутренние имена; числа и критерий стоят в «Почему такой вывод».
    ...
    override val journalEpisodes = "Записывать превышения в журнал"
    override val journalEpisodesNote =
        "Каждое превышение остаётся в Истории одной записью: начало, конец и пределы. " +
            "Выключено — тревога звучит по-прежнему, но записи не остаётся."
