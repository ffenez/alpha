 ...
 *
 * Живёт отдельно от рисования: попадание курсора в канал — утверждение о
 * данных и проверяется тестом. Рисование только применяет эти числа.
 *
 ...
 * бы не то, что показывает прибор.
 */
object SpectrumPlot {
     ...
     *
     * **Инженерный параметр**: треть высоты — размер, на котором пик виден
     * формой, и при этом под полем остаётся начало таблицы пиков.
     */
     ...
     *
     * В обоих последних случаях кривая рвётся: отрисовка у нижней границы
     * соединяла бы пустое место с соседними каналами и превращала спектр при
     * увеличении в частокол. Увеличение меняет горизонтальную подробность, а
     * не топологию кривой.
     */
     ...
     *
     * Колонка на экране — максимум нескольких каналов
     * ([SpectrumDisplay.aggregateMax]), поэтому курсор называет канал с
     * максимумом и возвращает список слитых каналов рядом.
     */
    fun readout(
        fraction: Float,
        range: IntRange,
