 ...
 * растянут по его значениям, мелкие различия видны, сравнение с другим
 * маршрутом теряет смысл. [MANUAL] — границы заданы вручную; названий вроде
 * «безопасно» у них нет.
 */
enum class MapColorScale { ABSOLUTE, ROUTE_CONTRAST, MANUAL }

/**
 * Границы ручной шкалы одной строкой: их до шести и они связаны порядком.
 * Разделителем принимается запятая, пробел или точка с запятой; десятичная
 * часть — точкой или запятой.
 */
object MapAnchors {

    /**
     * Запятая бывает и разделителем, и десятичным знаком («0,05,0,1» читается
     * двояко), поэтому правило деления задано заранее:
     *
     *  - есть пробел или «;» — они делят, запятая означает дробную часть
     *    («0,05 0,1 0,2» — три числа);
     *  - других разделителей нет — делит запятая, дробная часть пишется
     *    точкой («0.05,0.1,0.2»).
     *
     * Нечисловые куски в границы не попадают.
     */
...
object TrackMap {
    ...
    /**
     * Какая шкала красит след при выбранном режиме. Абсолютная требует
     * обычного фона места: пока его нет, шкала становится растянутой по
     * маршруту, и легенда называет режим.
     */
    fun scaleFor(
        mode: MapColorScale,
    ...
    /**
     * Шкала по заданным границам. Границ ровно [RAMP_STEPS]-1; значения
     * сортируются и очищаются от повторов — совпавшие границы теряют ступень,
     * а не ломают шкалу.
     */
    ...
    /** Границы ручной шкалы по умолчанию. */
     ...
     * координат не было. Прямая через такой пропуск — выдуманный маршрут.
     */
