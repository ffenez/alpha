...
data class LadderInput(
    val nowMillis: Long,
    /** Result of [app.alpha.analysis.RateComparison.compare]; null = none. */
 ...
 *    [app.alpha.analysis.RateComparison]. A window *differs* when the
 ...
 *    [app.alpha.analysis.RateComparison]); the confirmation-time idea is
 *    the same «magnitude AND duration» rule the profile baseline already uses
 *    ([app.alpha.baseline.PersistenceTracker], ADR 002).
