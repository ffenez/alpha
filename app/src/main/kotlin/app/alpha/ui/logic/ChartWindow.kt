...
object ChartWindows {
    ...
    /**
     * Лестница окон графика: 1-2-3-5-10-30 внутри каждой единицы времени, от
     * минуты до месяца. Ниже минуты ступеней нет — прибор пишет раз в секунду,
     * и в более коротком окне остаётся горстка отсчётов.
     */
     ...
     *
     * При секундной записи это триста измерений и колонка в пару секунд:
     * движение видно. На шестичасовом окне колонка около полутора минут, и
     * новая точка появляется раз в это время — картинка выглядит неподвижной.
     * Более длинные ступени выбираются вручную, выбор запоминается по величине.
     */
     ...
     *
     * ## Запас и отзывчивость жеста
     *
     * Сдвиг внутри прочитанного запроса не требует: снимок неизменен, меняется
     * проекция. Чем шире запас, тем дольше жест обходится без чтения базы.
     *
     * ## Запас не меняет ПУТЬ ЧТЕНИЯ
     *
     ...
     * почасовых скетчей. Поэтому у окна, которое читается точно, запас
     * ограничен остатком до границы: иначе метод сменился бы молча.
     */
    fun loadRange(
        window: ChartWindow,
        nowMillis: Long,
     ...
     *
     * Выбранная ступень — максимум, а не обещание, что данные за неё есть.
     * Левый край подтягивается к первому измерению, правый остаётся «сейчас»:
     * окно растёт вместе с историей и, дорастя до ступени, становится
     * скользящим.
     *
     * Без подтяжки у дозы менялся сам путь чтения: окно длиннее шести часов
     * уходит на почасовые скетчи (ADR 004), и короткая история складывалась в
     * одну часовую колонку, тогда как счёт и жёсткость показывали ряд.
     *
     * @param earliestMillis момент первого измерения; null — измерений нет,
     *   окно остаётся как выбрано.
    ...
    /**
     * Ступень лестницы, ближайшая к фактическому окну: щипок меняет окно
     * плавно, а лестница дискретна, и подсвеченный чип обязан называть то, что
     * на экране. Ближайшая ищется по ОТНОШЕНИЮ длительностей, а не по разности.
     */
     ...
     *
     * **Инженерный параметр**: 2 % ширины. Область правее «сейчас» не является
     * пропуском — там ещё нечего измерять, поэтому она не штрихуется и не
     * участвует в поиске пропусков.
     */
