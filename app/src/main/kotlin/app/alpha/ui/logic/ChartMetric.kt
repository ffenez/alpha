 ...
 *
 * Экран один на все три величины: жесты, окна, конверты, курсор, гистограмма
 * и правила отрисовки (разрывы не соединяются, экстремум не растягивает
 * конверт) от выбранной величины не зависят. Различаются источник данных,
 * единицы и набор доступных окон.
 */
...
object ChartMetrics {
     ...
     * скетчи), поэтому ей доступны окна до 30 дней. У счёта и жёсткости её
     * нет, а перебор миллионов сырых строк запрещён спецификацией графика
     * (§12): им доступны окна, которые точный путь читает целиком.
     * Ограничение названо в интерфейсе словами.
     */
     ...
     *
     * Без нижней границы размаха практически постоянный фон растягивается на
     * весь экран: шум ±0,002 мкЗв/ч выглядит как скачки. Числа — **инженерные
     * параметры отображения**: шаг, ниже которого разница не значима для
     * глаза (доза — 0,04 мкЗв/ч, счёт — около одной пуассоновской σ секундного
     * отсчёта фона, жёсткость — 0,05).
     */
     ...
     * полноэкранного графика: тап по карточке увеличивает ту же картинку.
     */
    fun startWindow(
        metric: ChartMetric,
