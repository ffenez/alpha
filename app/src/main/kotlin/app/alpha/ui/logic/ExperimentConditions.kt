...
/**
 * Условия опыта, разложенные на то, что можно повторить буквально.
 *
 * Одна строка «геометрия» позволяет повторить постановку лишь приблизительно,
 * а смысл A/B в том, что между прогонами меняется РОВНО ОДНО. Разложенные
 * условия показываются перед каждым следующим прогоном как список того, что
 * уже было.
 *
 * Приложение ничего не проверяет: расстояние и ориентацию оно знать не может.
 */
...
enum class ExperimentScenario(val kind: String) {
    ...
    /** A и B задаются вручную; шаблоны — расстояние, экранирование, другое. */
...
/** Как условия печатаются на экране. Логика чистая, экран только рисует. */
