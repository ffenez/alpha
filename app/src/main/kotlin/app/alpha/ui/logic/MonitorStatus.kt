...
sealed interface MonitorStatus {
    ...
    /**
     * Значение выше порога тревоги, но ещё не выдержало по длительности.
     * Отдельная ступень: тревога определяется величиной И длительностью,
     * поэтому ступень называет обе.
     */
    ...
    companion object {
        fun of(
            ...
            // Порог тревоги виден сразу, до выдержки, и независимо от того,
            // собран ли исторический диапазон места: это настройка, а не
            // статистика.
...
/**
 * Пояснение к выводу — показывается при включённых «Пояснениях на экранах».
 *
 * Здесь живёт сравнение «ниже вашего порога»: как вывод оно верно почти
 * всегда и потому ничего не сообщает, как пояснение — отвечает на вопрос,
 * с чем идёт сравнение, пока обычный фон не собран.
 *
