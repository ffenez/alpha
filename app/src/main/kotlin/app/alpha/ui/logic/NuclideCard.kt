 ...
 *
 * [energyKeV] отдаётся наружу: строка нажимается и показывает эту энергию
 * отметкой на спектре. [actionLabel] называет действие для экранного диктора.
 */
...
/**
 * Откуда взяты числа линий — одной строкой: числа справочной карточки обязаны
 * называть свой источник. Построчных неопределённостей в выборке ENSDF нет.
 */
 ...
 * ## Главное правило
 *
 * У карточки нет собственной логики «найдена / не найдена». Она печатает тот
 * самый [NuclideCheck], который [PeakEvidenceBridge] построил из вердикта
 * движка доказательств: читаются `classification`, `foundLines`,
 * `ambiguousWith` и поля каждой [CheckedLine]. Ни одна энергия здесь не
 * сравнивается заново.
 *
 * ## Границы формулировок (пинятся тестами)
 *
 *  - §12 — потолок «возможное совпадение»; «обнаружен» не пишется ни на одном
 *    языке, «не оценивалось» не превращается в «не найдено»;
 *  - §23 — ни безопасности, ни вреда, ни доз: карточка про нуклид;
 *  - отношения интенсивностей без кривой эффективности детектора в вывод не
 *    выносятся (`DetectorEfficiency.AVAILABLE = false`).
 */
object NuclideCard {
    ...
    private fun status(c: NuclideCheck?, s: NuclideStrings): NuclideStatusBlock {
        ...
        return when (c.classification) {
            EvidenceClass.CONTRADICTED -> {
                // Названы две линии: совпавшая и отсутствующая ожидаемая
                // заметная. Обе берутся из результата движка.
                val matched = c.lines
            ...
            // Одной линии мало, но причина зависит от нуклида: у K-40
            // гамма-линия одна и проверять совпадение нечем, у нуклида с
            // несколькими линиями на этом спектре проверить удалось одну.
            EvidenceClass.WEAK -> possible(
                counted = counted,
                // Считаются линии, о которых движок вынес суждение: найдена
                // или обязана была быть видна. Линия вне шкалы, слишком слабая
                // или без опорного континуума суждения не получила.
                tail = when {
    ...
    private fun lineRows(
        ...
        return c.lines.sortedByDescending { it.intensityPercent }.map { check ->
            ...
            // Измеренная энергия есть тогда, когда движок положил в проверку
            // найденный пик.
    ...
    /**
     * «Что усилило бы гипотезу»: конкретные ненайденные линии этого нуклида и
     * линии соседей по ряду распада, вычисленные из библиотеки.
     *
     * Короткий период полураспада сам по себе нуклид не подтверждает: спад
     * проверяется серией сопоставимых измерений при известных условиях.
     */
    ...
    /**
     * Provenance берётся из самих данных линий ([NuclideGammaLine.source]), а
     * не из подписи внизу карточки.
     */
    ...
    /**
     * ΔE со знаком: «−11,0», минус типографский (U+2212). Точность ΔE
     * ограничена шириной канала и калибровкой шкалы, поэтому число стоит рядом
     * с измеренной энергией.
     */
