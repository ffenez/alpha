...
object SpectrumHighlight {
     ...
     *
     * ИНЖЕНЕРНЫЙ ПАРАМЕТР, а не свойство измерения: столько времени хватает,
     * чтобы перевести взгляд со строки таблицы на картинку, прочитать подпись и
     * разглядеть место, — и не столько, чтобы забыть, откуда на поле взялась
     * вертикальная линия. Бессрочная отметка через минуту читалась бы как часть
     * графика, то есть как вывод о спектре.
     */
    ...
    data class Mark(
        ...
        /**
         * Что случилось с картинкой ради этой отметки. Хранится вместе с ней,
         * потому что подпись обязана сказать и «окно сдвинуто», и «этой линии
         * на шкале прибора нет» — молча подменить вид или молча ничего не
         * нарисовать одинаково нечестно.
         */
        val outcome: Aim,
    )
     ...
     *
     * Ширина окна НЕ меняется: кратность зума выбрал человек, и подсветка линии
     * не повод её отбирать. Окно только едет — и лишь тогда, когда энергия
     * действительно за краем (или в его margin-полоске).
     *
     * Энергия за пределами шкалы прибора не «почти видна»: такое окно не
     * существует, отметке негде стоять, и вместо картинки честнее сказать это
     * словами ([Aim.OUT_OF_SCALE]).
     */
