...
data class HistorySelection(
    ...
    /**
     * «Выбрать всё» и «снять всё» одним действием: кнопка переключается по
     * тому, выбрано ли уже всё.
     *
     * Идущая сессия в список не попадает — её нельзя удалить, и счётчик
     * выбранного не должен её считать.
     */
...
/**
 * Что именно удаляется и что об этом сказано в подтверждении.
 *
 * План удаления и набор целей — один объект: удалить не то, что посчитано,
 * нельзя. Пока они ездили порознь, удаление одной записи из её «⋮» строило
 * план по ней, а стирало выбранное в режиме выбора, то есть ничего.
 */
