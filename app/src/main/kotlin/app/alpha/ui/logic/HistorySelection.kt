...
/**
 * Что именно удаляем и что об этом сказано человеку.
 *
 * Раньше по экрану ездили отдельно план удаления (для текста подтверждения) и
 * отдельно набор выбранных записей (для самого удаления). Совпадали они только
 * при удалении из режима выбора: удаление одной записи из её «⋮» строило план
 * по ней, а стирало ВЫБРАННОЕ — то есть ничего, потому что в режиме выбора
 * никто не был. Кнопка нажималась, окно закрывалось, запись оставалась.
 *
 * Теперь план и цели — один объект: удалить не то, что посчитано, нельзя.
 */
data class PendingDeletion(
    val plan: DeletionPlan,
    val sessions: Set<Long> = emptySet(),
    val spectra: Set<Long> = emptySet(),
    /** Удаление начато из режима выбора — по завершении его нужно снять. */
