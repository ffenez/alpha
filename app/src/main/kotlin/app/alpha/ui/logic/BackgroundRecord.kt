...
import app.alpha.ui.text.BackgroundCardRu
import app.alpha.ui.text.BackgroundCardStrings
 ...
 * самом перечислении: она зависит от языка интерфейса.
 */
...
/**
 * Карточка записанного фона (ТЗ §10) как модель, а не как набор строк в
 * композиции: состав первого уровня проверяется тестом.
 */
data class BackgroundCardModel(
    ...
    /** Фон пригоден: карточка ничего не требует. */
...
object SearchBaseline {
    ...
    fun proposal(
        ...
        // Называется факт и условие сравнения, без предположений о том, что
        // происходило эти полчаса.
     ...
     *
     * Первый уровень отвечает на три вопроса: какой фон записан, когда и как
     * долго его мерили и — если он больше не годится — одной строкой, что
     * именно с ним не так. Развёрнутое объяснение лежит в
     * [BackgroundCardModel.details].
     *
    ...
    fun card(
        ...
        return BackgroundCardModel(
            ...
            basis = c.recordedAt(day, timeOfDay),
            reason = reason,
            action = c.refresh,
            // Пока фон годится, кнопка не является основным действием.
            actionPrimary = check != BackgroundCheck.USABLE,
            details = listOfNotNull(
                // Развёрнутое объяснение причины — уровнем ниже.
