...
class ReleaseNotesTest {
    ...
    fun `the newest note describes the build that is installed`() {
        ...
        assertEquals(app.alpha.BuildConfig.VERSION_NAME, ReleaseNotes.current)
    }
    ...
    fun `version numbers do not depend on the language`() {
        ...
        val ru = ReleaseNotes.notes(app.alpha.ui.text.ReleaseRu).map { it.version }
        val en = ReleaseNotes.notes(app.alpha.ui.text.ReleaseEn).map { it.version }
        assertEquals(ru, en)
        assertEquals(ru.first(), ReleaseNotes.current)
        assertEquals(ReleaseNotes.SHOWN, ReleaseNotes.shownIn(app.alpha.ui.text.ReleaseEn).size)
    }
