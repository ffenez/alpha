...
class BackupStringsTest {
    ...
    fun `каждая причина отказа названа словами`() {
        for (catalogue in listOf(BackupRu, BackupEn)) {
            val problems = listOf(
                app.radiacode.data.export.backup.BackupProblem.NotABackup,
                app.radiacode.data.export.backup.BackupProblem.TooNew(2, 1),
                app.radiacode.data.export.backup.BackupProblem.Missing("measurements"),
                app.radiacode.data.export.backup.BackupProblem.Corrupted("measurements"),
                app.radiacode.data.export.backup.BackupProblem.Unreadable("нет доступа"),
            )
