...
class AcceptedResolutionTest {
    ...
    fun `peak search follows the accepted model and returns to the approximation`() {
        val approximate = app.radiacode.analysis.PeakDetection.expectedFwhmKeV(1460.8f)
        ResolutionSource.install(record)
        val measured = app.radiacode.analysis.PeakDetection.expectedFwhmKeV(1460.8f)
        assertTrue(
            kotlin.math.abs(measured - approximate) > 1f,
            "измеренная ширина обязана отличаться: $measured против $approximate",
        )
        ResolutionSource.install(null)
        assertEquals(approximate, app.radiacode.analysis.PeakDetection.expectedFwhmKeV(1460.8f))
    }

    @Test
    fun `the radon ROI keeps the approximation so its series stays comparable`() {
        val before = app.radiacode.analysis.PeakDetection.fwhmKeV(609.3f)
        ResolutionSource.install(record)
        assertEquals(before, app.radiacode.analysis.PeakDetection.fwhmKeV(609.3f))
    }
}
