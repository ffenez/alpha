 ...
 *
 * [truncated] means the payload ended inside a record body: everything before
 * it is decoded and valid, the tail is unrecoverable. Отдельный признак нужен,
 * потому что «ответ пришёл неполным» и «ответ разобран целиком» — разные
 * состояния связи, и первое обязано быть видно в отчёте.
 */
...
object DataBufDecoder {
    ...
    fun decode(payload: ByteArray, baseTimeMillis: Long): DataBufResult {
        ...
        // Обрыв внутри тела записи не отменяет уже разобранные записи: каждая
        // прочитана целиком, и терять из-за хвоста весь ответ нельзя.
        try {
            while (r.remaining() >= 7) {
                val seq = r.readU8()
                val eid = r.readU8()
            ...
            return DataBufResult(ret, seqGaps, truncated = true)
        }
